...
export function ServiceCard({ service, index = 0 }: ServiceCardProps) {
  return (
    <article className="service-card reveal" style={{ '--delay': `${index * 90}ms` } as React.CSSProperties}>
      ...
      <a href={getWhatsAppLink(`Hello Sundas Beauty Parlour, I would like to book ${service.name}.`)} className="service-link">Book on WhatsApp <span aria-hidden="true">↗</span></a>
    </article>
  );
}
